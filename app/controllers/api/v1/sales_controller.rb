class Api::V1::SalesController < ApplicationController
  ...
  def index
    if @current_account.present?
      ...
      render json: @sales, include: { account: { only: %i[id name address] } }
    else
  ...
  def show
    if @current_account.present?
      render json: @sale, include: { account: { only: %i[id name address] } }
    else
      render json: { error: 'Unauthorized' }, status: :unauthorized
    end
  end
  ...
  def purchases_granularity_search
    if @current_account.present?
      ...
      render json: { error: 'Invalid granularity param' }, status: :bad_request and return unless %i[hour day week month year].include?(granularity)
  ...
  def update
    if (@current_account.present? && @current_account.role_level.zero?) || @current_account.role_level == 1
  ...
  # Trusted params
  def sale_params
    params.require(:sale).permit(:product_amount, :total_price, :purchased_at, :admin_id, :product_id, :category_id,
                                 :account_id)
  end
end
